@php
    use Carbon\Carbon;
@endphp
<x-dashboard-layout title="Links">

    <div class="w-full bg-white  p-2 rounded-lg">
        <form action="{{ route('links.store') }}" method="POST" class="relative">
            @csrf
            <div class="relative">
                <input type="text" name="original_url"
                    ...
                    placeholder="https://yourdomain.id/very-long-link" required>
                <button type="submit"
            ...
            @enderror
            <div id="more-input" class="hidden">
                <div>
                    <h1 class="text-sm font-bold my-4">Title (optional)</h1>
                    <input name="title" type="text"
                        class="w-full rounded border-gray-300 text-sm placeholder:text-gray-400" placeholder="Title">
                </div>
                <div>
                    <h1 class="text-sm font-bold my-4">Deskripsi (optional)</h1>
                    <textarea name="deskripsi" class="w-full rounded border-gray-300 text-sm placeholder:text-gray-400"
                        placeholder="Deskripsi"></textarea>
                </div>
                <div class="flex flex-col lg:flex-row mt-4">
                    <div class="lg:border-r w-full lg:w-[20rem] lg:pr-4">
                        <div class="space-y-2">
                            <button id="protected-btn" type="button"
                                class="w-full p-2 bg-blue-100 text-blue-600 rounded text-sm flex items-center gap-2">
                                <i class="fa-solid fa-lock"></i>
                                <p class="font-medium">Protected Link</p>
                            </button>
                            <button id="time-based-btn" type="button"
                                class="w-full p-2 bg-gray-100 text-gray-600 hover:bg-gray-300 rounded text-sm flex items-center gap-2">
                                <i class="fa-solid fa-clock"></i>
                                <p class="font-medium">Time-based Link</p>
                            </button>
                        </div>
                    </div>
                    <div class="flex-1 lg:p-4 lg:pr-24">
                        <div id="protected" class="block mt-4 lg:mt-0"> <!-- Tampilkan bagian protected -->
                            <h1 class="font-semibold">Protected Link</h1>
                            <div class="w-full p-2 bg-gray-100 border flex items-start lg:items-center gap-2 my-3">
                                <i class="fa-solid fa-circle-info text-sm"></i>
                                <p class="text-sm">A protected link can be given a secret key/passphrase for access
                                    before being redirected to the original link</p>
                            </div>
                            <div class="w-full relative lg:max-w-[20rem] mb-2 lg:mb-0">
                                <input type="password" id="password-input" name="password"
                                    class="w-full text-sm placeholder:text-gray-400 border-gray-300 rounded-lg"
                                    placeholder="Password">
                                <i id="toggle-password"
                                    class="fa-solid fa-eye absolute right-0 top-1/2 -translate-y-1/2 text-gray-400 cursor-pointer p-3 hover:text-blue-600 transition"></i>
                            </div>
                        </div>
                        <div id="time-based" class="hidden mt-4 lg:mt-0">
                            <h1 class="font-semibold">Time-based Link</h1>
                            <div class="w-full p-2 bg-gray-100 border flex items-start lg:items-center gap-2 my-3">
                                <i class="fa-solid fa-circle-info text-sm"></i>
                            ...
                            <div class="w-full flex flex-col gap-4 mb-2 lg:mb-0">
                                <div class="flex w-full lg:max-w-[20rem] flex-col gap-1">
                                    <label for="expiration_date" class="w-full mb-2 lg:mb-0">Expired Date & Time</label>
                                    <input type="datetime-local" id="expiration_date" name="expiration_date"
                                        class="text-sm placeholder:text-gray-400 border-gray-300 rounded-lg">
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="w-full mt-1">
            <button id="show-more-btn" type="button"
                class="w-full flex items-center gap-2 justify-center hover:bg-gray-200 transition bg-gray-100 rounded p-1 lg:p-2">
                <p class="text-sm">Show more</p>
                <i class="fa-solid fa-angle-down mt-1"></i>
            </button>
            <button id="hidden-show-more-btn" type="button"
                class="w-full items-center gap-2 justify-center hover:bg-gray-200 transition bg-gray-100 rounded p-1 lg:p-2 hidden">
    ...
    <div class="w-full mt-6 flex flex-col lg:flex-row gap-2 lg:gap-0 lg:items-center justify-between">
        ...
        <form action="{{ route('links.index') }}" method="GET" class="relative">
                ...
                <div class="w-full bg-white rounded-lg relative"> <!-- Tambahkan class relative di sini -->
                    <div class="w-full p-4 border rounded-t-lg lg:flex items-start justify-between">
                        <div class="w-full overflow-hidden">
                            ...
                            <h3 onclick="salinTeksKeClipboard('{{ env('APP_URL') . '/' . $link->short_url }}')"
                                ...
                                <div id="share-admin-{{ $link->id }}"
                                    ...
                                    <ul>
                                        <li class="flex flex-col items-start px-1 py-1">
                                            ...
                                            <div class="w-full hover:bg-gray-200 mb-2">
                                                <a href="https://api.whatsapp.com/send?text={{ urlencode(env('APP_URL') . '/' . $link->short_url) }}"
                                                    target="_blank" class="flex items-center px-2 py-1 w-full">
                        ...
                        <div class="flex items-center gap-2 text-sm text-gray-400 font-semibold">
                            ...
                            <div class="flex items-center gap-2">
                                <p class="text-sm font-normal text-gray-500">
                                    {{ date('d M Y H:i', strtotime($link->created_at)) }}</p>
                            </div>
                        </div>
                        <div class="w-fit flex items-center gap-2">
                            @if ($link->expiration_date && $link->expiration_date > now())
                                <button id="set-time-btn-{{ $link->id }}" title="Time-based Link"
                                    class="flex items-center gap-2 p-2 px-3 bg-green-100 rounded hover:bg-green-200 transition text-green-600 text-xs lg:text-sm">
                                    <i class="fa-solid fa-clock"></i>
                                    <p class="hidden lg:block">
                                        {{ Carbon::parse($link->expiration_date)->diffForHumans() }}</p>
                                </button>
                            @elseif ($link->expiration_date && $link->expiration_date < now())
                                <button id="set-time-btn-{{ $link->id }}" title="Time-based Link"
                                    class="flex items-center gap-2 p-2 px-3 bg-red-200 rounded hover:bg-blue-600 hover:text-white transition text-red-700 text-xs lg:text-sm">
                                    <i class="fa-solid fa-clock"></i>
                                    <p class="hidden lg:block">Expired</p>
                                </button>
                            @else
                                <button id="set-time-btn-{{ $link->id }}" title="Time-based Link"
                                    class="flex items-center gap-2 p-2 px-3 bg-gray-300 rounded hover:bg-blue-600 hover:text-white transition text-gray-700 text-xs lg:text-sm">
                                    <i class="fa-solid fa-clock"></i>
                                    <p class="hidden lg:block">Set Time</p>
                                </button>
                            @endif
                            @if ($link->password)
                                <button id="pashphraseBtn-{{ $link->id }}" title="Protected Link"
                                    class="flex items-center gap-2 p-2 px-3 bg-blue-100 rounded hover:bg-blue-200 text-blue-600 transition  text-xs lg:text-sm">
                                    <i class="fa-solid fa-lock"></i>
                                    <p class="hidden lg:block">Locked</p>
                                </button>
                            @else
                                <button id="pashphraseBtn-{{ $link->id }}" title="Protected Link"
                                    class="flex items-center gap-2 p-2 px-3 bg-gray-300 rounded hover:bg-blue-600 hover:text-white transition text-gray-700 text-xs lg:text-sm">
                                    <i class="fa-solid fa-lock-open"></i>
                                    <p class="hidden lg:block">Set Password</p>
                                </button>
                            @endif
                        ...
                        <div class="chart-container">
                            <div class="container mx-auto lg:p-4 mb-4">
                                <div class="flex justify-between items-center mb-4">
                                    ...
                                    {{-- <form method="GET" action="3" class="ml-auto flex items-center"> <!-- ml-auto untuk memindahkan form ke kanan -->
                                        <label for="tahun" class="mr-2">Pilih Tahun:</label> <!-- Label untuk dropdown -->
                        ...
                        <div class="w-[30rem] bg-white p-4 shadow-lg rounded-lg">
                            ...
                            @if ($link->expiration_date)
                                <div class="flex flex-col gap-1 text-sm mt-2 p-2 border bg-teal-100 text-teal-700">
                                    <h2 class=" font-bold">Time-based link has been set</h2>
                                    <p>Link will expire at
                                        <span
                                            class="font-semibold">{{ date('d F Y, H:i', strtotime($link->expiration_date)) }}</span>
                                    </p>
                                </div>
                            @endif
                            <form action="{{ route('links.updateExpired', $link->id) }}" method="POST"
                                ...
                                <div
                                    ...
                                    <div class="flex items-center justify-end gap-3">
                                        ...
                                        <button type="submit"
                                            class="py-2 px-4 rounded-lg font-medium bg-blue-600 text-white hover:bg-blue-700 transition">
                                            @if ($link->expiration_date)
                                                Change
                                            @else
                                                Save
    ...
    <script>
        ...
        function salinTeksKeClipboard(text) {
            navigator.clipboard.writeText(text).then(function() {
                    tampilkanAlert("Teks berhasil disalin!"); // Tampilkan alert saat teks berhasil disalin
                },
                function(err) {
        ...
        function showTimeModal(id) {
            ...
            const modal = document.getElementById(`time-based-modal-${id}`);
            if (modal) {
        ...
        // Loop untuk menambahkan event listener ke setiap button modal open/close
        document.addEventListener('DOMContentLoaded', function() {
            // Ambil semua tombol "Set Time" berdasarkan id
            @foreach ($links as $link)
                const openBtn{{ $loop->iteration }} = document.getElementById(`set-time-btn-{{ $link->id }}`);
                const closeBtns{{ $loop->iteration }} = document.querySelectorAll(
                    `#closeTimeModal-{{ $loop->iteration }}`);

                // Event listener untuk membuka modal saat tombol diklik
                openBtn{{ $loop->iteration }}.addEventListener('click', function() {
                    showTimeModal({{ $loop->iteration }});
                });

                // Event listener untuk menutup modal saat tombol close diklik
                closeBtns{{ $loop->iteration }}.forEach(closeBtn => {
